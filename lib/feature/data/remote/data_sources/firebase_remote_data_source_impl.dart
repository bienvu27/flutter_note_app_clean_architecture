...
import 'package:note_app_clean_architecture/feature/data/remote/models/note_model.dart';
import 'package:note_app_clean_architecture/feature/data/remote/models/user_model.dart';
...
class FirebaseRemoteDataSourceImpl implements FirebaseRemoteDataSource {
  ...
  Future<void> addNewNote(NoteEntity noteEntity) async {
    ...
    noteCollectionRef.doc(noteId).get().then((note) {
      final newNote =
          NoteModel(uid: noteEntity.uid, noteId: noteId, note: noteEntity.note, time: noteEntity.time).toDocument();
      if (!note.exists) {
  ...
  Future<void> getCreateCurrentUser(UserEntity user) async {
    final userConllectionRef = firestore.collection("users");
    final uid = await getCurrentUId();
    userConllectionRef.doc(user.uid).get().then((value) {
      ...
      if (!value.exists) {
        userConllectionRef.doc(uid).set(newUser);
      }
      return;
    });
  }
  ...
  Stream<List<NoteEntity>> getNotes(String uid) {
    final noteConllectionRef = firestore.collection("users").doc(uid).collection("notes");
    return noteConllectionRef.snapshots().map((querySnap) {
  ...
  @override
  Future<void> signIn(UserEntity user) => auth.signInWithEmailAndPassword(email: user.email!, password: user.password!);
  ...
  Future<void> updateNote(NoteEntity note) async {
    ...
    final noteConllectionRef = firestore.collection("users").doc(note.uid).collection("notes");
    if (note.note != null) noteMap['note'] = note.note;
    if (note.time != null) noteMap['time'] = note.time;

    noteConllectionRef.doc(note.noteId).update(noteMap);
  }
}
